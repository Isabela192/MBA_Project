...
class TransferCommand(Command):
    ...
    def execute(self, session: Session) -> Dict[str, Any]:
        from_statement = select(Account).where(Account.account_id == self.from_account_id)
        ...
        if from_account.balance < self.amount:
            raise ValueError(f"FAILED! Insufficient funds in account {self.from_account_id}")
